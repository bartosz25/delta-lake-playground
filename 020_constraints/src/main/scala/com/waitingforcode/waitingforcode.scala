...
package object waitingforcode {
  ...
  val OutputDir = "/tmp/delta-lake-playground/020_constraints"
  ...
  def createTable(tableName: String, constraintLetter: String = "",
                 constraintNumber: String = ""): Unit = {
    sparkSession.sql(s"DROP TABLE IF EXISTS ${tableName}")
    sparkSession.sql(
      s"""CREATE TABLE ${tableName} (
         |  letter STRING ${constraintLetter}, number INT ${constraintNumber}
         |) USING DELTA
         |""".stripMargin)
  }
